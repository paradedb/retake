...
/// Convert a memory string like `"16GB"` into bytes
...
/// Convert bytes to a user-friendly string, e.g. `12345678 -> "11.77MB"`
...
/// Main configuration for this benchmark suite
...
/// Info about how we set maintenance_work_mem, storing relevant stats
#[derive(Debug, Serialize, Deserialize)]
...
/// Data from the custom index creation step
#[derive(Debug, Serialize, Deserialize)]
...
/// Sample of system resource usage (CPU, memory, disk I/O) during indexing
#[derive(Debug, Serialize, Deserialize)]
...
pub struct PgBenchTestResult {
    ...
    // from any "progress:" lines in stdout
    pub intervals: Vec<PgBenchLogInterval>,

    // from the 6-column lines in `pgbench_log.*`
    pub transaction_log: Vec<PgBenchTransactionLog>,

    // from the 15-column lines in `pgbench_log.*` if aggregate-interval is used
...
/// A 6-column line from `-l` logs (per transaction)
#[derive(Debug, Serialize, Deserialize)]
...
/// A 15-column line from aggregated logs, e.g.:
///   interval_start num_transactions sum_latency sum_latency_2 min_latency
///   max_latency sum_lag sum_lag_2 min_lag max_lag skipped retried
///   retries serialization_failures deadlock_failures
#[derive(Debug, Serialize, Deserialize)]
...
impl BenchmarkSuite {
    /// Create the suite: connect to Postgres, ensure the report table, etc.
    ...
    async fn benchmark_index_creation(&mut self) -> Result<IndexCreationBenchmarkResult> {
        ...
        let handle = std::thread::spawn(move || -> Vec<SysinfoSample> {
            let mut sys = System::new_all();
            let mut samples = vec![];
            while !sf_clone.load(Ordering::SeqCst) {
                sys.refresh_all();
    ...
    /// If user included `--progress=1`, parse "progress: 1.0 s, 101.0 tps, lat 12.3 ms ..."
    ...
    /// Parse lines from the aggregated `-l --aggregate-interval` logs:
    /// If a line has 15 columns => it is a "15‐column" aggregated interval row
    /// If it has 6 columns => it is a "6‐column" transaction row
                ...
                if let Ok(content) = fs::read_to_string(&path) {
                    for line in content.lines() {
                        ...
                        if parts.len() == 15 {
                            ...
                            // maybe it's a CSV line with 6 columns:
                            let csv_parts: Vec<_> = line.split(',').collect();
                            if csv_parts.len() == 6 {
                                // parse per-transaction
    ...
    /// parse a 15-column aggregated line from `--aggregate-interval` logs
    fn parse_15_column_aggregated_line(parts: &[&str]) -> Result<PgBenchAggregateIntervalRow> {
        // We expect each of these to be i64
        if parts.len() != 15 {
            bail!("Not a valid 15-column aggregated line");
        }
        // Column order is documented as:
        //   interval_start, num_transactions, sum_latency, sum_latency_2,
        //   min_latency, max_latency, sum_lag, sum_lag_2,
        //   min_lag, max_lag, skipped, retried, retries,
        //   serialization_failures, deadlock_failures
        let row = PgBenchAggregateIntervalRow {
    ...
    /// parse a 6‐column per‐transaction line
    fn parse_6_column_transaction_line(csv_parts: &[&str]) -> Result<PgBenchTransactionLog> {
        // Format: thread_id, client_id, xact_id, time_secs, latency_secs, stmt_name
        // We only store some of them
        if csv_parts.len() != 6 {
            bail!("Not a valid 6-column transaction line");
        }
    ...
    /// Actually run pgbench on one `.sql`
    async fn run_single_sql_pgbench(
        ...
        let sql_content = fs::read_to_string(sql_file)
            .with_context(|| format!("Could not read {}", sql_file.display()))?;

        // ephemeral combined .sql
        ...
        cmd.current_dir(scratch_path)
            ...
            .arg("--no-vacuum")
            // logs each transaction + aggregated lines:
            .arg("-l")
        ...
        // parse logs in scratch dir (both 6-col and 15-col lines)
        ...
        for line in stdout_str.lines() {
            if let Some(idx) = line.find("tps =") {
                ...
                if let Some(space_idx) = after.find(' ') {
                    if let Ok(val) = after[..space_idx].trim().parse::<f64>() {
                        tps = Some(val);
                    }
                }
            } else if let Some(idx) = line.find("latency average =") {
                let after = &line[idx + 17..].trim();
                if let Some(space_idx) = after.find(' ') {
                    if let Ok(val) = after[..space_idx].trim().parse::<f64>() {
                        latency = Some(val);
                    }
    ...
    pub async fn run_all_benchmarks(&mut self) -> Result<()> {
        ...
        let dir_entries = fs::read_dir(&self.config.sql_folder)
            .with_context(|| format!("Could not read folder {:?}", self.config.sql_folder))?;
        for entry in dir_entries {
            let path = entry?.path();
            if path.extension().and_then(|e| e.to_str()) == Some("sql") {
                println!("Running pgbench for SQL file: {}", path.display());
    ...
    /// Insert final JSON into the table
